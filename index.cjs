  ...
  switch (baseType) {
    ...
    case "string":
      ...
      if (column instanceof import_pg_core.PgDateString) {
        return isInput ? { type: import_graphql2.GraphQLString, description: "Date" } : { type: import_graphql_scalars.GraphQLDate, description: "Date" };
      }
    ...
    case "number": {
      ...
      if (dims !== void 0 && dims > 0) {
        ...
        const baseType2 = baseDesc === "Integer" ? import_graphql2.GraphQLInt : import_graphql2.GraphQLFloat;
        return {
          type: new import_graphql2.GraphQLList(new import_graphql2.GraphQLNonNull(baseType2)),
...
var resolveGenericFilterName = (column, columnName, columnGraphQLType) => {
  ...
  if (columnGraphQLType.type instanceof import_graphql3.GraphQLList) {
...
var generateTableTypes = (tableName, tables, relationMap, withReturning, relationsDepthLimit, cacheCtx) => {
  ...
  const arrTableItemOutput = withReturning ? (
    ...
    new import_graphql3.GraphQLNonNull(new import_graphql3.GraphQLList(new import_graphql3.GraphQLNonNull(selectSingleOutput)))
  ) : void 0;
  const inputs = {
    insertInput,
    updateInput,
